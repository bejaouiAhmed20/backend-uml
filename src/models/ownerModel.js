...
const updateOwner = (id, name, email, phone, password, res) => {
  ...
  if (password) {
    updateQuery = 'UPDATE owner SET name = ?, email = ?, phone = ?, password = ? WHERE id = ?';
    params = [name, email, phone, password, id];
  } else {
    updateQuery = 'UPDATE owner SET name = ?, email = ?, phone = ? WHERE id = ?';
...
//   const updateQuery = `UPDATE owner SET name = ?, email = ?, phone = ? WHERE id = ?`;
...
module.exports = { findOwnerByEmail, addOwner, findOwnerById, updateOwner,getAllOwners,deleteOwnerById};
